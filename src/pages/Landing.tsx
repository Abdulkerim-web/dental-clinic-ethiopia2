    ...
    <div>
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="container-page relative grid grid-cols-1 items-center gap-12 py-16 sm:py-24 lg:grid-cols-[1.1fr_0.9fr]">
          <div>
            <span className="inline-flex items-center rounded-full border border-teal-700/20 bg-teal-700/5 px-4 py-1.5 text-xs font-semibold uppercase tracking-wide text-teal-700 dark:text-teal-200">
              {t(lang, 'hero_eyebrow')}
            </span>
            <h1 className="mt-5 font-display text-4xl font-medium leading-[1.08] text-teal-900 dark:text-teal-50 sm:text-6xl">
              {t(lang, 'hero_title')}
            </h1>
            ...
            <dl className="mt-14 grid grid-cols-3 gap-6 border-t border-teal-700/10 pt-8">
              {stats.map((s) => (
                <div key={s.label}>
                  <dt className="font-display text-2xl font-semibold text-teal-800 dark:text-teal-100 sm:text-3xl">{s.value}</dt>
          ...
          <div className="relative mx-auto aspect-[4/5] w-full max-w-sm">
            ...
            <div className="absolute inset-6 flex flex-col justify-between rounded-[36%_36%_6%_6%/26%_26%_6%_6%] border border-white/40 bg-white/40 p-6 backdrop-blur-sm">
              <div className="rounded-2xl bg-surface/90 p-4 shadow-soft">
                <p className="text-xs font-semibold text-teal-700 dark:text-teal-200">Next appointment</p>
                <p className="mt-1 font-display text-lg font-semibold text-teal-900 dark:text-teal-50">Tue, Jun 23 · 10:30</p>
      ...
      {/* Services */}
      <section id="services" className="border-t border-teal-700/10 bg-surface py-20">
        <div className="container-page">
          <div className="max-w-2xl">
            <h2 className="font-display text-3xl font-medium text-teal-900 dark:text-teal-50 sm:text-4xl">
          ...
          <div className="mt-12 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {SERVICES.map((s, i) => (
              <div key={i} className="card flex flex-col gap-3">
                ...
                <h3 className="font-display text-lg font-semibold text-teal-900 dark:text-teal-50">{s[lang].name}</h3>
      ...
      <section className="bg-sand-50 py-20">
        <div className="container-page flex flex-col items-center justify-between gap-6 rounded-3xl border border-teal-700/10 bg-surface px-8 py-12 text-center shadow-soft sm:flex-row sm:text-left">
          <div>
            <h3 className="font-display text-2xl font-medium text-teal-900 dark:text-teal-50 sm:text-3xl">
